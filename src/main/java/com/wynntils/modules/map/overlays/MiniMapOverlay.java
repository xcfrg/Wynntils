...
public class MiniMapOverlay extends Overlay {
    ...
    public void render(RenderGameOverlayEvent.Pre e) {
        ...
        // texture position
        float minX = map.getTextureXPosition(McIf.player().getX()) - extraFactor * (mapSize/2f + zoom);  // <--- min texture x point
        float minZ = map.getTextureZPosition(McIf.player().getZ()) - extraFactor * (mapSize/2f + zoom);  // <--- min texture z point

        float maxX = map.getTextureXPosition(McIf.player().getX()) + extraFactor * (mapSize/2f + zoom);  // <--- max texture x point
        float maxZ = map.getTextureZPosition(McIf.player().getZ()) + extraFactor * (mapSize/2f + zoom);  // <--- max texture z point
        ...
        try {
            ...
            if (MapConfig.INSTANCE.minimapIcons) {
                ...
                final int minFastWorldX = (int) (McIf.player().getX() - extraFactor * (mapSize/2f + zoom));
                final int minFastWorldZ = (int) (McIf.player().getZ() - extraFactor * (mapSize/2f + zoom));

                final int maxFastWorldX = (int) (McIf.player().getX() + extraFactor * (mapSize/2f + zoom)) + 1;
                final int maxFastWorldZ = (int) (McIf.player().getZ() + extraFactor * (mapSize/2f + zoom)) + 1;
                ...
                Consumer<MapIcon> consumer = c -> {
                    ...
                    float dx = (float) (posX - McIf.player().getX()) * scaleFactor;
                    float dz = (float) (posZ - McIf.player().getZ()) * scaleFactor;
                ...
                if (compassIcon.isEnabled(true)) {
                    float dx = (float) (compassIcon.getPosX() - McIf.player().getX()) * scaleFactor;
                    float dz = (float) (compassIcon.getPosZ() - McIf.player().getZ()) * scaleFactor;
            ...
            if (MapConfig.INSTANCE.showCoords) {
                drawString(
                        String.join(", ", Integer.toString((int) McIf.player().getX()), Integer.toString((int) McIf.player().getY()), Integer.toString((int) McIf.player().getZ())),
