    ...
    private void locateSecretDiscovery(String name, String action) {
        ...
        handler.addAndDispatch(query.handleJsonObject(jsonOutput -> {
            if (jsonOutput.has("error")) { // Returns error if page does not exist
                Minecraft.getMinecraft().player.sendMessage(new StringTextComponent(TextFormatting.RED + "Unable to find discovery coordinates. (Wiki page not found)"));
                return true;
            }
            ...
            try {
                ...
                Minecraft.getMinecraft().player.sendMessage(new StringTextComponent(TextFormatting.RED + "Unable to find discovery coordinates. (Wiki template not located)"));
                return true;
            }

            if (x == 0 && z == 0) {
                Minecraft.getMinecraft().player.sendMessage(new StringTextComponent(TextFormatting.RED + "Unable to find discovery coordinates. (Wiki coordinates not located)"));
                return true;
            }
